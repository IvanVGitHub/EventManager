...
//        ImageIcon imageIcon = new ImageIcon(byteImageBase64);
        ...
        for (int i = 0; i < getCountCameras(); i++) {
            ...
            ArrayList<ImageIcon> image = QueryEventPicture.imageIcon(idCamera);

            //add event to group event
            for(int a = 0; a < QueryEventPicture.imageIcon(idCamera).size(); a++) {
                //random color border event for TEST
                Color randomColor = new Color(rand.nextFloat(), rand.nextFloat(), rand.nextFloat());
                eventAdd.createEventLabel("Камера " + i, labelSize, randomColor, image.get(a));
            }
